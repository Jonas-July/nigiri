...
struct index_mapping {
  ...
  auto map(attribute_idx_t const& i) const { return i != attribute_idx_t::invalid() ? i + attribute_idx_offset_ : attribute_idx_t::invalid(); }
  auto map(bitfield_idx_t const& i) const { return i != bitfield_idx_t::invalid() ? bitfield_idx_map_[i] : bitfield_idx_t::invalid(); }
  auto map(booking_rule_idx_t const& i) const { return i != booking_rule_idx_t::invalid() ? i + booking_rule_idx_offset_ : booking_rule_idx_t::invalid(); }
  ...
  auto map(merged_trips_idx_t const& i) const { return i != merged_trips_idx_t::invalid() ? i + merged_trips_idx_offset_ : merged_trips_idx_t::invalid(); }
  auto map(provider_idx_t const& i) const { return i != provider_idx_t::invalid() ? i + provider_idx_offset_ : provider_idx_t::invalid(); }
  ...
  auto map(string_idx_t const& i) const { return string_idx_map_[i]; }
  ...
  auto map(area const& i) const { return area{.id_=map(i.id_), .name_=map(i.name_)}; }
  auto map(booking_rule const& i) const { return booking_rule{
                                            .id_ = map(i.id_),
                                            .type_ = i.type_,
  ...
  auto map(fares::fare_media const& i) const { return fares::fare_media{.name_ = map(i.name_), .type_=i.type_}; }
  auto map(fares::fare_product const& i) const { return fares::fare_product{
                                                   .amount_ = i.amount_,
                                                   .name_ = map(i.name_),
                                                   .media_ = i.media_,
  ...
  auto map(fares::rider_category const& i) const { return fares::rider_category{
                                                     .name_ = map(i.name_),
                                                     .eligibility_url_ = map(i.eligibility_url_),
                                                     .is_default_fare_category_ = i.is_default_fare_category_};
                                                 }
  auto map(fares::timeframe const& i) const { return fares::timeframe{.start_time_=i.start_time_, .end_time_=i.end_time_, .service_=i.service_, .service_id_=map(i.service_id_)}; }
  auto map(fares::network const& i) const { return fares::network{.id_=map(i.id_), .name_=map(i.name_)}; }
  ...
  auto map(provider const& i) const { return provider{
                                        .id_ = map(i.id_),
                                        .name_ = map(i.name_),
                                        .url_ = map(i.url_),
                                        .tz_ = map(i.tz_),
...
void timetable::merge(timetable const& other_tt) {
  ...
  //fwd_search_lb_graph_ not used during loading
  ...
  this->route_bikes_allowed_.resize(route_bikes_allowed_size + new_route_bikes_allowed_size);
  for (auto const& i : vw::iota(0U, new_route_bikes_allowed_size)) {
    this->route_bikes_allowed_.set(route_bikes_allowed_size + i, new_route_bikes_allowed.test(i));
  }
  ...
  this->route_cars_allowed_.resize(route_cars_allowed_size + new_route_cars_allowed_size);
  for (auto const& i : vw::iota(0U, new_route_cars_allowed_size)) {
    this->route_cars_allowed_.set(route_cars_allowed_size + i, new_route_cars_allowed.test(i));
  }
  auto const route_stop_times_offset = this->route_stop_times_.size();
  for (auto const& i : new_route_stop_time_ranges) {
    this->route_stop_time_ranges_.push_back(interval{i.from_ + route_stop_times_offset, i.to_ + route_stop_times_offset});
  }
  ...
  for (auto const& n : new_flex_area_rtree.nodes_) {
    if (n.kind_ == rtree<flex_area_idx_t>::kind::kLeaf) {
      for (size_t i = 0; i < n.count_; ++i) {
        this->flex_area_rtree_.insert(n.rects_[i].min_, n.rects_[i].max_, n.data_[i]);
      }
    }
  }
  ...
  for (auto const& i : new_trip_transport_ranges) {
    this->trip_transport_ranges_.emplace_back(i);
  }
