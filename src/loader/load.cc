...
timetable load(std::vector<timetable_source> const& sources,
  ...
  for (auto const [idx, in] : utl::enumerate(sources)) {
    ...
    if (it != end(loaders)) {
      ...
      auto const new_route_bikes_allowed_per_section = other_tt.route_bikes_allowed_per_section_;
      auto const new_route_cars_allowed_per_section = other_tt.route_cars_allowed_per_section_;
      ...
      auto const new_transport_first_dep_offset = other_tt.transport_first_dep_offset_;
      ...
      auto const new_transport_to_trip_section = other_tt.transport_to_trip_section_;
      ...
      auto const new_transport_section_attributes = other_tt.transport_section_attributes_;
      auto const new_transport_section_providers = other_tt.transport_section_providers_;
      auto const new_transport_section_directions = other_tt.transport_section_directions_;
      auto const new_transport_section_lines = other_tt.transport_section_lines_;
      auto const new_transport_section_route_colors = other_tt.transport_section_route_colors_;
      ...
      auto const new_location_group_transports = other_tt.location_group_transports_;
      auto const new_flex_area_transports = other_tt.flex_area_transports_;
      auto const new_flex_transport_traffic_days = other_tt.flex_transport_traffic_days_;
      auto const new_flex_transport_trip = other_tt.flex_transport_trip_;
      auto const new_flex_transport_stop_time_windows = other_tt.flex_transport_stop_time_windows_;
      ...
      auto const new_flex_transport_pickup_booking_rule = other_tt.flex_transport_pickup_booking_rule_;
      auto const new_flex_transport_drop_off_booking_rule = other_tt.flex_transport_drop_off_booking_rule_;
      ...
      {// merge locations struct
        ...
        for (auto const& i : new_locations.location_id_to_idx_) {
          auto loc_id = i.first;
          loc_id.src_ = loc_id.src_ != source_idx_t::invalid() ? loc_id.src_ + source_idx_offset : source_idx_t::invalid();
          auto const loc_idx = i.second != location_idx_t::invalid() ? i.second + locations_offset : location_idx_t::invalid();
        ...
        for (auto const& i : new_locations.alt_names_) {
          ...
          for (auto const& j : i) {
            vec.push_back(j != alt_name_idx_t::invalid() ? j + alt_name_idx_offset : alt_name_idx_t::invalid());
          }
        }
        ...
        for (auto const& i: new_locations.src_) {
          loc.src_.push_back(i != source_idx_t::invalid() ? i + source_idx_offset : source_idx_t::invalid());
        }
        ...
        for (auto const& i: new_locations.parents_) {
          loc.parents_.push_back(i != location_idx_t::invalid() ? i + locations_offset : location_idx_t::invalid());
        }
        for (auto const& i: new_locations.location_timezones_) {
          loc.location_timezones_.push_back(i != timezone_idx_t::invalid() ? i + timezones_offset : timezone_idx_t::invalid());
        }
        for (auto const& i : new_locations.equivalences_) {
          auto entry = loc.equivalences_.emplace_back();
          for (auto const& j : i) {
            auto loc_idx = j != location_idx_t::invalid() ? j + locations_offset : location_idx_t::invalid();
        ...
        for (auto const& i : new_locations.preprocessing_footpaths_out_) {
          ...
          for (auto const& j : i) {
            auto fp = footpath{j.target() != location_idx_t::invalid() ? j.target() + locations_offset : location_idx_t::invalid(), j.duration()};
      ...
      for (auto const& i : new_location_areas) {
        ...
        for (auto const& j : i) {
          vec.push_back(j != area_idx_t::invalid() ? j + area_idx_offset : area_idx_t::invalid());
        }
      }
      for (location_idx_t i = location_idx_t{0}; i < location_idx_t{new_location_location_groups.size()}; ++i) {
        ...
        for (auto const& j : new_location_location_groups[i]) {
          tt.location_location_groups_.back().push_back(j != location_group_idx_t::invalid() ? j + location_group_offset : location_group_idx_t::invalid());
        }
      }
      for (location_group_idx_t i = location_group_idx_t{0}; i < location_group_idx_t{new_location_group_locations.size()}; ++i) {
        ...
        for (auto const& j : new_location_group_locations[location_group_idx_t{i}]) {
          tt.location_group_locations_.back().push_back(j != location_idx_t::invalid() ? j + locations_offset : location_idx_t::invalid());
        }
      }
      ...
      for (auto const& i : new_route_transport_ranges) {
        tt.route_transport_ranges_.push_back(interval{i.from_ != transport_idx_t::invalid() ? i.from_ + transport_idx_offset : transport_idx_t::invalid(),
                                                      i.to_   != transport_idx_t::invalid() ? i.to_   + transport_idx_offset : transport_idx_t::invalid()
                                                     });
      }
      for (auto const& i : new_route_location_seq) {
        ...
        for (auto const& j : i) {
          auto const s = stop{j};
          auto const mapped_location_idx = s.location_idx() != location_idx_t::invalid() ? s.location_idx() + locations_offset : location_idx_t::invalid();
          auto const mapped_stop = stop{mapped_location_idx, s.in_allowed_, s.out_allowed_, s.in_allowed_wheelchair_, s.out_allowed_wheelchair_};
      ...
      tt.route_bikes_allowed_.resize(route_bikes_allowed_size + new_route_bikes_allowed_size);
      ...
      tt.route_cars_allowed_.resize(route_cars_allowed_size + new_route_cars_allowed_size);
      ...
      for (auto const& i : new_fares) {
        ...
        for (auto const& j : i.fare_media_) {
          auto const mapped_media = fares::fare_media{
            .name_ = string_map[j.name_],
            .type_ = j.type_
          };
          mapped_fare_media.push_back(mapped_media);
        }
        auto mapped_fare_products = vecvec<fare_product_idx_t, fares::fare_product>{};
        ...
        auto mapped_fare_product_id = vector_map<fare_product_idx_t, string_idx_t>{};
        ...
        for (auto const& j : i.fare_leg_rules_) {
          auto const mapped_rule = fares::fare_leg_rule{
            ...
            .network_ = j.network_,
            .from_area_ = j.from_area_ != area_idx_t::invalid() ? j.from_area_ + area_idx_offset : area_idx_t::invalid(),
            .to_area_   = j.to_area_   != area_idx_t::invalid() ? j.to_area_   + area_idx_offset : area_idx_t::invalid(),
        ...
        for (auto const& j : i.fare_leg_join_rules_) {
          auto const mapped_join_rule = fares::fare_leg_join_rule{
            ...
            .from_stop_ = j.from_stop_ != location_idx_t::invalid() ? j.from_stop_ + locations_offset : location_idx_t::invalid(),
            .to_stop_   = j.to_stop_   != location_idx_t::invalid() ? j.to_stop_   + locations_offset : location_idx_t::invalid()
          };
          mapped_fare_leg_join_rules.push_back(mapped_join_rule);
        }
        auto mapped_rider_categories = vector_map<rider_category_idx_t, fares::rider_category>{};
        ...
        for (auto const& j : i.timeframes_) {
          ...
          for (auto const& k : j) {
            auto const mapped_timeframe = fares::timeframe{
              .start_time_ = k.start_time_,
        ...
        auto mapped_timeframe_id = vector_map<timeframe_group_idx_t, string_idx_t>{};
        ...
        for (auto const& j : i.area_sets_) {
          ...
          for (auto const& k : j) {
             vec.push_back(k != area_idx_t::invalid() ? k + area_idx_offset : area_idx_t::invalid());
          }
        }
        ...
        auto const mapped_fares = fares{
          .leg_group_name_ = mapped_leg_group_name,
      ...
      for (auto const& i : new_providers) {
        auto const p = provider{
                  .id_ = string_map[i.id_],
                  .name_ = string_map[i.name_],
                  .url_ = string_map[i.url_],
                  .tz_ = i.tz_ != timezone_idx_t::invalid() ? i.tz_ + timezones_offset : timezone_idx_t::invalid(),
                  .src_ = i.src_ != source_idx_t::invalid() ? i.src_ + source_idx_offset : source_idx_t::invalid()};
        tt.providers_.push_back(p);
      }
      for (auto const& i : new_provider_id_to_idx) {
        tt.provider_id_to_idx_.push_back(i != provider_idx_t::invalid() ? i + provider_idx_offset : provider_idx_t::invalid());
      }
      /*	  Flex		*/
      ...
      for (auto const& i : new_flex_area_src) {
        tt.flex_area_src_.push_back(i != source_idx_t::invalid() ? i + source_idx_offset : source_idx_t::invalid());
      }
      ...
      for (auto const& i : new_flex_transport_trip) {
        tt.flex_transport_trip_.push_back(i != trip_idx_t::invalid() ? i + trip_offset : trip_idx_t::invalid());
      }
      ...
      for (auto const& i : new_flex_transport_pickup_booking_rule) {
        ...
        for (auto const& j : i) {
          vec.push_back(j != booking_rule_idx_t::invalid() ? j + booking_rules_offset : booking_rule_idx_t::invalid());
        }
      }
      ...
      for (auto const& i : new_trip_id_to_idx) {
        tt.trip_id_to_idx_.push_back(pair<trip_id_idx_t, trip_idx_t>{i.first  != trip_id_idx_t::invalid() ? i.first  + trip_id_offset : trip_id_idx_t::invalid(),
                                                                     i.second != trip_idx_t::invalid()    ? i.second + trip_offset    : trip_idx_t::invalid()
                                                                    });
      }
      for (auto const& i : new_trip_ids) {
        auto entry = tt.trip_ids_.emplace_back();
        for (auto const& j : i) {
          auto trip_id = trip_id_idx_t{j != trip_id_idx_t::invalid() ? j + trip_id_offset : trip_id_idx_t::invalid()};
          entry.emplace_back(trip_id);
        }
      }
      for (auto const& i : new_trip_id_src) {
        tt.trip_id_src_.push_back(i != source_idx_t::invalid() ? i + source_idx_offset : source_idx_t::invalid());
      }
      ...
      for (auto const& i : vw::iota(0U, to_idx(add_size))) {
        ...
        tt.trip_direction_id_.set(idx + trip_offset, new_trip_direction_id.test(idx));
      }
      for (auto const& i : new_trip_route_id) {
        tt.trip_route_id_.push_back(i);
      }
      for (auto const& i : new_trip_transport_ranges) {
      ...
      for (auto const& i : new_merged_trips) {
        ...
        for (auto const& j : i) {
          vec.push_back(j != trip_idx_t::invalid() ? j + trip_offset : trip_idx_t::invalid());
        }
      }
      /*      route_id_idx_t	 */
      for (auto const& i : new_route_ids) {
        auto vec = paged_vecvec<route_id_idx_t, trip_idx_t>{};
        for (auto const& j : i.route_id_trips_) {
        ...
        for (auto const& j : i.route_id_provider_) {
          mapped_providers.push_back(j != provider_idx_t::invalid() ? j + provider_idx_offset : provider_idx_t::invalid());
        }
      ...
      for (auto const& i : new_transport_traffic_days) {
        tt.transport_traffic_days_.push_back(i != bitfield_idx_t::invalid() ? corrected_indices[i] : bitfield_idx_t::invalid());
      }
      for (auto const& i : new_transport_to_trip_section) {
        auto vec = tt.transport_to_trip_section_.add_back_sized(0U);
        for (auto const& j : i) {
          vec.push_back(j != merged_trips_idx_t::invalid() ? j + merged_trips_idx_offset : merged_trips_idx_t::invalid());
        }
      }
      ...
      for (auto const& i : new_transport_section_providers) {
        ...
        for (auto const& j : i) {
          vec.push_back(j != provider_idx_t::invalid() ? j + provider_idx_offset : provider_idx_t::invalid());
        }
      }
      ...
      for (auto const& i : new_transport_section_lines) {
        ...
        for (auto const& j : i) {
          vec.push_back(j != trip_line_idx_t::invalid() ? j + trip_lines_offset : trip_line_idx_t::invalid());
        }
      }
      ...
      for (auto const& i : new_attribute_combinations) {
        ...
        for (auto const& j : i) {
          vec.push_back(j != attribute_idx_t::invalid() ? j + attribute_idx_offset : attribute_idx_t::invalid());
        }
      }
