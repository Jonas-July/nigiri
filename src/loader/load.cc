...
timetable load(std::vector<timetable_source> const& sources,
  ...
  for (auto const [idx, in] : utl::enumerate(sources)) {
    ...
    if (it != end(loaders)) {
      ...
      for (auto const& i : vw::iota(old_route_bikes_allowed.size(), tt.route_bikes_allowed_.size())) {
      ...
      for (auto const& i : vw::iota(old_route_cars_allowed.size(), tt.route_cars_allowed_.size())) {
      ...
      assert(tt.route_bikes_allowed_per_section_.size() == tt.route_location_seq_.size());
      assert(old_route_bikes_allowed_per_section.size() == old_route_location_seq.size());
      assert(old_route_bikes_allowed_per_section.size() <= tt.route_bikes_allowed_per_section_.size());
      for (auto const& i : vw::iota(old_route_bikes_allowed_per_section.size(), tt.route_bikes_allowed_per_section_.size())) {
        new_route_bikes_allowed_per_section.emplace_back(tt.route_bikes_allowed_per_section_[route_idx_t{i}]);
      }
      auto new_route_cars_allowed_per_section = vecvec<route_idx_t, bool>{};
      assert(tt.route_cars_allowed_per_section_.size() == tt.route_location_seq_.size());
      assert(old_route_cars_allowed_per_section.size() == old_route_location_seq.size());
      assert(old_route_cars_allowed_per_section.size() <= tt.route_cars_allowed_per_section_.size());
      for (auto const& i : vw::iota(old_route_cars_allowed_per_section.size(), tt.route_cars_allowed_per_section_.size())) {
        new_route_cars_allowed_per_section.emplace_back(tt.route_cars_allowed_per_section_[route_idx_t{i}]);
      }
      ...
      for (auto const& i : vw::iota(route_offset_bikes, new_route_bikes_allowed_size)) {
      ...
      for (auto const& i : vw::iota(route_offset_cars, new_route_cars_allowed_size)) {
      ...
      for (auto const& i : new_trip_id_to_idx) {
        tt.trip_id_to_idx_.push_back(pair<trip_id_idx_t, trip_idx_t>{i.first != trip_id_idx_t::invalid() ? i.first + trip_id_offset : trip_id_idx_t::invalid(), i.second});
      }
      ...
      assert(tt.trip_train_nr_.size() == 0);
      /* 	 trip_idx_t	 */
      auto const trip_direction_offset = trip_idx_t{tt.trip_direction_id_.size()};
