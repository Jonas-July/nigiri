...
void shapes_storage::add(shapes_storage* other) {
  ...
  for (auto const& e : other->trip_offset_indices_) {
    auto const mapped_shape_idx        = e.first  !=        shape_idx_t::invalid() ? e.first  +        shape_size :        shape_idx_t::invalid();
    auto const mapped_shape_offset_idx = e.second != shape_offset_idx_t::invalid() ? e.second + shape_offset_size : shape_offset_idx_t::invalid();
    auto const mapped_trip_offset_index = cista::pair<shape_idx_t, shape_offset_idx_t>{mapped_shape_idx, mapped_shape_offset_idx};
