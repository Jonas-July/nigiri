...
timetable load(std::vector<timetable_source> const& sources,
  ...
  for (auto const [idx, in] : utl::enumerate(sources)) {
    ...
    if (it != end(loaders)) {
      ...
      auto shape_store = shapes != nullptr ? std::make_unique<shapes_storage>(local_cache_path, shapes->mode_) : nullptr;
      ...
      auto const new_route_bikes_allowed_per_section = tt.route_bikes_allowed_per_section_;
      auto const new_route_cars_allowed_per_section = tt.route_cars_allowed_per_section_;
      auto const new_route_stop_time_ranges = tt.route_stop_time_ranges_;
      auto const new_route_stop_times = tt.route_stop_times_;
      auto const new_transport_first_dep_offset = tt.transport_first_dep_offset_;
      ...
      auto const new_transport_section_attributes = tt.transport_section_attributes_;
      auto const new_transport_section_providers = tt.transport_section_providers_;
      auto const new_transport_section_directions = tt.transport_section_directions_;
      auto const new_transport_section_lines = tt.transport_section_lines_;
      auto const new_transport_section_route_colors = tt.transport_section_route_colors_;
      ...
      auto const new_flex_transport_pickup_booking_rule = tt.flex_transport_pickup_booking_rule_;
      auto const new_flex_transport_drop_off_booking_rule = tt.flex_transport_drop_off_booking_rule_;
