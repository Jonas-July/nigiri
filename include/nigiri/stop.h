...
struct stop {
  ...
  static constexpr auto const kMaxLocation =
      std::numeric_limits<location_idx_t::value_t>::max() >> (kTotalBits - kLocationBits);
  ...
  location_idx_t location_idx() const {
    auto const location_idx = location_ != kMaxLocation ? location_idx_t{location_} : location_idx_t::invalid();
    return location_idx;
  }
